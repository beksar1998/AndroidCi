#Скрипт выгрузки бандла приложения в GOOGLE PlayMarket
##Вызывается из задачи выполнения скрипта оболочки в конвейере Pipeline

Формат вызова:


```
echo "Starting Deploy BASH script"

key=/data/vsts/api-5720474703527235039-161088-af9e40aba2d2.json
bundle=/data/vsts/_work/2/a/app/build/outputs/bundle/productionRelease/app-production-release.aab
target=internal
draft=false
version_code=$(cat /data/vsts/_work/2/s/code_vers.txt)

/data/vsts/scripts/deploy_aab.sh "$(cat $key)" $bundle $target $version_code $draft
```

#Место положение скрипта
...
PACKAGE_NAME='com.tallium.eubank.smartbank'
